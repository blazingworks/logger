...
test(`Info Log`, () => {
    ...
    expect(logs).toEqual([
        { message: "My name is Sakuko Akiyama!", level: LogLevel.Info, fullMessage: "[info] My name is Sakuko Akiyama!" },
    ]);
});
...
test(`Fatal Log`, () => {
    ...
    expect(logs).toEqual([
        { message: "My name is Gen Minegishi!", level: LogLevel.Fatal, fullMessage: "[fatal] My name is Gen Minegishi!" },
    ]);
});
